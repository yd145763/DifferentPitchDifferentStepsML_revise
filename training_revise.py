...
for steps in Steps:
    ...
    X = np.moveaxis(X, [0], [2])
    X_train = np.array(X)
    X_train = np.reshape(X_train, (int(X_train.shape[0]), int(X_train.shape[2]), int(X_train.shape[1])))
    ...
    #set validation data
        ...
        X_test = np.array(X1)
        X_test = np.reshape(X_test, (int(X_test.shape[0]), int(X_test.shape[2]), int(X_test.shape[1])))
        ...
        X_test2 = np.array(X2)
        X_test2 = np.reshape(X_test2, (int(X_test2.shape[0]), int(X_test2.shape[2]), int(X_test2.shape[1])))
        ...
        y_pred_ori_list.append(y_pred_ori)
        y_test_ori_list.append(y_test_ori)
    ...
    df_result['training_loss_list'] = training_loss_list
    df_result['validation_loss_list'] = validation_loss_list
    df_result['y_pred_ori_list'] = y_pred_ori_list
    df_result['y_test_ori_list'] = y_test_ori_list
